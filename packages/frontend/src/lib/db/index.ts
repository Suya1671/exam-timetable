import type { SqlError, SqlParam, SqlQueryResult, SqlValue } from '@exam-timetable/tauri-api'
...
function normalizeRows(rows: SqlQueryResult['rows']) {
    ...
    return rows.map(row => row.map(cell => fromSqlValue(cell)))
}
...
export const db = drizzle(
    async (sql, params, method) => {
        ...
        if (method === 'get') {
            // Weird workaround
            return { rows: normalizedRows[0] ?? (undefined as unknown as any[]) }
        }
