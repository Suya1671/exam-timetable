<script lang="ts">
    ...
    import { Icon, DatePickerDocked, easeEmphasized } from "m3-svelte";
...
<div style="display: flex;flex-direction: column;gap: 0.25rem;">
    ...
    >
        <input
            ...
            name={field.name}
            bind:value={
                () => field.state.value.toISOString().substring(0, 10),
                (v) => (field.state.value = v)
            }
        ...
        {#if picker}
            <div class="picker" transition:enterExit>
                <DatePickerDocked
                    date={field.state.value.toISOString().substring(0, 10)}
                    clearable={!required}
                    close={() => (picker = false)}
                    setDate={(d) => {
                        if (input) {
