...
<script lang="ts">
	...
	const batchApply = (scope: 'all' | 'slot1' | 'slot2') => {
		const targets =
			scope === 'all' ? allTimeslots : scope === 'slot1' ? slotOneTimeslots : slotTwoTimeslots;
...
>
	...
	<div class="info-section">
		...
		<p class="hint">
			Left click toggles one slot. Right click clears. Drag paints a date range. Click a day number
			to toggle the whole day.
		</p>
	</div>
	...
	<div class="calendar-grid">
		{#each visibleCells as cell (cell.date.getTime())}
			<article class="day-cell" class:outside={!cell.inMonth} data-day-key={dateKeyUTC(cell.date)}>
					...
					<div class="split-circle" role="group" aria-label="Timeslots for date">
						{#if top}
							...
							<button
								...
								onpointerdown={(event) => onSlotPointerDown(event, top.id, dateKeyUTC(cell.date))}
