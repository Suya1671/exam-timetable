<script lang="ts">
	...
	const importCsvForm = createForm(() => ({
		...
		onSubmit: async ({ value }) => {
			if (!csvImportState || csvImporting) return;

			csvImporting = true;
			try {
				const allowedForGrade = await subjectOptionsForGrade(value.grade);
				const allowedIds = new Set(
					allowedForGrade.map((option) => Number(option.value)).filter((id) => Number.isFinite(id))
				);
				...
				for (const row of csvImportState.rows) {
					...
					for (const csvSubjectName of row.subjects) {
						...
						if (mappedSubjectId) finalSubjectIds.add(mappedSubjectId);
					}
					...
					for (const subjectId of finalSubjectIds) {
						await db.insert(enrolledStudent).values({ studentId, subjectId }).onConflictDoNothing();
					}
				}
	...
	function toGradeBoundedNumber(value: unknown) {
		...
		if (!Number.isFinite(n)) return DEFAULT_GRADE;
	...
	async function subjectOptionsForGrade(grade: number): Promise<SelectOption[]> {
		if (!Number.isFinite(grade) || grade < MIN_GRADE || grade > MAX_GRADE) return [];
	...
	async function findExistingStudentIdByCsvRow(row: CsvImportPreviewRow) {
		...
		if (studentName.length === 0) return null;

		const existing = await db
			.select({ id: student.id })
			.from(student)
			.where(
				or(eq(student.name, studentName), eq(student.name, `${row.surname} ${row.name}`.trim()))
			)
			.limit(1);
	...
	async function buildCsvImportState(
		...
		const csvSubjectNames = [...new Set(parsedRows.flatMap((row) => row.subjects))].sort((a, b) =>
			a.localeCompare(b)
		);
	...
	async function handleCsvFilePicked(file: File | null) {
		...
		const exactByNameInsensitive = new Map(
			(csvImportState.defaultSubjectOptions ?? []).map((s) => [s.text.toLocaleLowerCase(), s.value])
		);
		...
		<form id="import-csv-form" onsubmit={(e) => handleFormSubmit(e, importCsvForm)}>
			...
			{#if csvImportState}
				...
				<fieldset>
					...
					<p>
						Map each subject in the CSV file to a subject in the system. Use Skip to ignore a
						subject.
					</p>
							...
							<ListItem headline={csvSubjectName}>
								{#snippet trailing()}
									<div class="mapping-trailing">
										...
										<importCsvForm.Field name={`mappings[${index}].csvSubjectName`}>
											{#snippet children(field)}
												<input type="hidden" name={field.name} value={csvSubjectName} />
											{/snippet}
										</importCsvForm.Field>
									</div>
								{/snippet}
