<script lang="ts">
	...
	import { Button, Select, Snackbar, TextField, snackbar } from 'm3-svelte';
	...
	const gridFilter = $derived.by(() => {
		...
		return (row: IRow) => {
			...
			const matchesGrade =
				gradeFilter === null || (typeof row.grade === 'number' && row.grade === gradeFilter);
	...
	async function handleRemoveSubjectEvent(ev: RemoveSubjectEvent) {
		if (!confirm(`Remove "${ev.subjectName}" from this student?`)) return;
	...
	async function handleDeleteStudentEvent(ev: DeleteStudentEvent) {
		if (!confirm(`Delete "${ev.studentName}"?`)) return;
	...
	async function handleClearSubjectsEvent(ev: ClearSubjectsEvent) {
		if (!confirm(`Clear all subjects from "${ev.studentName}"?`)) return;
...
<main>
	<form aria-label="Filter students" onsubmit={(event) => event.preventDefault()}>
		...
		<Select
			...
			bind:value={
				() => (gradeFilter === null ? 'all' : gradeFilter.toString()),
