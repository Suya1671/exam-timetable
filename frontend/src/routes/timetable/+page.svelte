<script lang="ts">
	...
	function describeInitSolverError(error: InitSolverError): string {
		if ('LockPoison' in error) return error.LockPoison;
		if ('Solver' in error) return error.Solver;
		if ('Recv' in error) return error.Recv;
	...
	function describeSolveSessionControlError(error: SolveSessionControlError): string {
		if ('LockPoison' in error) return error.LockPoison;
		if ('Send' in error) return error.Send;
		if ('InvalidSessionId' in error) return `Invalid session id ${error.InvalidSessionId}`;
	...
	function buildLockedSlots(): LockedTimetableSlot[] {
		...
		for (const sessionId of lockedSessions) {
			...
			if (timeslotId === undefined) continue;
			lockedSlots.push({
	...
	async function saveCurrentTimetable() {
		...
		await db.transaction(async (tx) => {
			await tx.delete(timetableSlots).where(eq(timetableSlots.timetableId, selectedTimetableId!));
			if (rows.length > 0) {
	...
	function moveExam(sessionId: number, timeslotId: number, grade: number) {
		...
		if (!session || session.grade !== grade || lockedSessions.has(sessionId)) return;
	...
	async function toggleStream() {
		if (solveSessionId === null || solveDone) return;
		try {
...
<main>
	<section class="no-print" aria-label="Solution navigation and status">
		<menu aria-label="Solution pager">
			...
			<li>
				<Button onclick={nextVariant} disabled={variantIndex >= variants.length - 1}>Next</Button>
			</li>
		</menu>
