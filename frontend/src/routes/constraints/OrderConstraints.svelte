<script lang="ts">
	...
	async function getPaperOptions(subjectId: string, grade: string) {
		...
		if (!Number.isFinite(parsedSubject) || !parsedGrade) return [];
	...
	const form = createForm(() => ({
		...
		onSubmit: async ({ value, formApi }) => {
			...
			if (!Number.isFinite(exam1Id) || !Number.isFinite(exam2Id) || exam1Id === exam2Id) return;
		...
		<form id="order-constraint-form" onsubmit={handleSubmit}>
			<div class="exams">
				<fieldset>
					...
					<form.Subscribe selector={(s) => ({ g: s.values.grade1, s: s.values.subject1Id })}>
				...
				<fieldset>
					...
					<form.Subscribe selector={(s) => ({ g: s.values.grade2, s: s.values.subject2Id })}>
						{#snippet children(state)}
							<form.Field
								name="exam2Id"
								validators={{ onChange: pipe(string(), nonEmpty('Pick paper')) }}
				...
				<table>
					...
					<tbody>
						{#each loaded as c (c.exam1Id + '-' + c.exam2Id)}
							<tr>
								<td
									>{formatExamLabel(c.firstExam)} must be before {formatExamLabel(c.secondExam)}</td
								>
								<td
									><Button variant="text" color="error" onclick={() => remove(c.exam1Id, c.exam2Id)}
										>Delete</Button
									></td
								>
							</tr>
